...
struct Data* Sort(struct Data* data, int split, int n)
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n-i-1;j++) //attention not n-i
        { 
            a = data[j].point[split];
            b = data[j+1].point[split];
            if(a>b)
            {
                temp = data[j];
                data[j] = data[j+1];
...
struct Data* uniondata(struct Data* data, struct Data* data1,double point[], int K)
{
    for(int i=0;i<K;i++)
    {
        for(int j=0;j<K;j++)
        {
            if(distance(point,data1[j].point)<distance(point,data[i].point))
            {
                data[i].state = data1[j].state;
                data[i].county = data1[j].county;
                data[i].point[0] = data1[j].point[0];
                data[i].point[1] = data1[j].point[1];
            }
        }
    }
    sortdata(data,point,K);
...
struct Node* build(struct Node *root,struct Data* data,unsigned depth,int n) 
{
    ...
    {
        Rightdata[i] = data[mid+i+1];  //mid put into right tree
    }
...
*/
struct Node* newNode(struct Data data,int split) 
{ 
    struct Node* temp = new Node; 
    temp->isLeaf = true;
    temp->split = split;
    temp->pivot = data.point[split];
    temp->state = data.state; 
    temp->county = data.county; 
    temp->lat = data.point[0]; 
    temp->lon = data.point[1]; 
    temp->left = NULL;
    temp->right = NULL;
    return temp; 
} 
  
Node *insert(Node *root, struct Data data, unsigned depth) 
{ 
    unsigned split = depth % k; 

    if (root == NULL) 
       return newNode(data,split); 

    root->isLeaf = false;
  
    if (data.point[split] < (root->pivot)) 
    {
...
{ 
    ...
    for(int i=0;i<1600000;i++)
    {
        ...
        int len = res.size();
        inputdata.state = res[0];
        inputdata.county = res[1];
        if(len>4)
        {
            for(int j=2;j<len-2;j++)
            {
                inputdata.county += " "+res[j];
            }
        }
        inputdata.point[0] = stod(res[len-2]);
        inputdata.point[1] = stod(res[len-1]);
        N = N+1;
        tree = insert(tree, inputdata, 0);
        res.clear();
    }

    cout<<"two thirds built"<<endl;
    while (!fin.eof())
    {
        getline(fin,str);
        if(str.empty())
        ...
        inputdata.point[1] = stod(res[len-1]);
        N = N+1;
        tree1 = insert(tree1, inputdata, 0);
        res.clear();
    }
    fin.close();
    tree->parent = NULL;
    ...
    cout<<"building time is "<<building<<endl;
    ...
    if(K==1)
    {
      ...
      ans1 = searchNN(point,tree1);
      stop = chrono::steady_clock::now();
      if(distance(ans.point,point)<distance(ans1.point,point))
      {
      ...
      {
        cout<<ans1.state<<" "<<ans1.county<<" "<<endl;
        cout<<ans1.point[0]<<" "<<ans1.point[1]<<endl;
      }
    }
    else
    {
      ...
      answer1 = search(point,tree1,K);
      answer = uniondata(answer,answer1,point,K);
      stop = chrono::steady_clock::now();
      for(int j=0;j<K;j++)
      {
    ...
    while(1)
    {
        ...
        K = 5;
        struct Data* answers;
        struct Data* answers1;
        answers = search(point,tree,K);
        answers1 = search(point,tree1,K);
        answers = uniondata(answers,answers1,point,K);
