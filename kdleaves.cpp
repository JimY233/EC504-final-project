...
struct Node* build(struct Node *root,struct Data* data,unsigned depth,int n) 
{
    ...
    {
        Rightdata[i] = data[mid+i];  //mid put into right tree
    }
...
struct Data searchNN(double point[], struct Node* root)
{
    ...
    else
    {
        if(current->parent->left == NULL)
          ...
          detect = current->parent->left;
    }
    //detect->visited = true;

    while(detect->parent != NULL)
    {
        unsigned dim = detect->split;
        double rectdist = fabs(detect->pivot - point[dim]);
        if(rectdist < mindist)
        {
            struct Node *detectdown;
            detectdown = searchdown(point,detect);
            double possible[2];
            possible[0] = detect->lat;
            possible[1] = detect->lon;
            double possibledist = distance(point,possible);
        ...
        if(detect->parent->parent != NULL)
        {
            if(detect->parent->isLeft)
            detect = detect->parent->parent->right;
            else
            detect = detect->parent->parent->left;
        }
        else
        {
            detect = detect->parent;
        }
    }
...
struct Data* search(double point[], struct Node* root, int K)
{
    ...
    {
        detectdown = searchdown(point,detect);
        knearestdata[i].state = detectdown->state;
        knearestdata[i].county = detectdown->county;
        knearestdata[i].point[0] = detectdown->lat;
        knearestdata[i].point[1] = detectdown->lon;
        if(detect->parent->parent != NULL)
        {
            if(detect->parent->isLeft)
              detect = detect->parent->parent->right;
    ...
    while(detect->parent!=NULL)
    {
        ...
        detectdown = searchdown(point,detect);
        possible.state = detectdown->state;
        possible.county = detectdown->county;
        possible.point[0] = detectdown->lat;
        possible.point[1] = detectdown->lon;
